...
class Likes(models.Model):
    ...
    def user_unliked_post(sender, instance, *args, **kwargs):
        ...
        post = like.post 
        sender = like.user 
        notify = Notification.objects.filter(post=post, sender=sender, notification_tpes=1)
        notify.delete() 
        """
...
class Follow(models.Model):
    ...
    def user_unfollow(sender, instance, *args, **kwargs):
        ...
        sender = follow.user
        following = follow.following
        # notify = Notification.objects.filter(sender=sender, user=following, notificatin_types=3)
        # notify.delete()
    ...
    def add_post(sender, instance, created, *args, **kwargs ):
        if created:
            ...
            for follower in followers:
                print("count1~", follower.user.username)
...
post_save.connect(Stream.add_post, sender=Post, dispatch_uid="my_unique_identifier")
